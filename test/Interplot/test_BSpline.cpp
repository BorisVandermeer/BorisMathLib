...
void EventMouseClick(int event, int x, int y, int flags, void* ustc)
{
    ...
    if (event == EVENT_RBUTTONDOWN)
    {
        ...
        curmode = showing;
        if(x_s.size()>2){
            vector<double> points(x_s.size()*2);
        ...
        double length = 0;
        for (size_t i = 0; i != x_s.size() - 1; ++i) {
            length += sqrt((x_s[i]-x_s[1+i])*(x_s[i]-x_s[1+i])+(y_s[i]-y_s[1+i])*(y_s[i]-y_s[1+i]));
        }
        double tmp_t = 0;
        double delta_t = 1.0 / length;
        while (tmp_t < 1) {
            auto result = bsp.eval(tmp_t).result();
            if(result[0]>0&&result[0]<IMG_WIDTH&&result[1]>0&&result[1]<IMG_HIGHT){
                img.at<Vec3b>(result[1],result[0]) = IMG_COLOR;
            }
            tmp_t += delta_t;
        }
