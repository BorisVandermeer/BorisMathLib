...
namespace Dubins{
    ...
    int dubins_word(DubinsIntermediateResults* in, DubinsSpace::DubinsPathType pathType, double out[3]) {
        int result;
        switch(pathType) {
        case DubinsSpace::LSL:
            result = dubins_LSL(in, out);
            break;
        case DubinsSpace::RSL:
            result = dubins_RSL(in, out);
            break;
        case DubinsSpace::LSR:
            result = dubins_LSR(in, out);
            break;
        case DubinsSpace::RSR:
            result = dubins_RSR(in, out);
            break;
        case DubinsSpace::LRL:
            result = dubins_LRL(in, out);
            break;
        case DubinsSpace::RLR:
...
using namespace Dubins;
    ...
    int DubinsSpace::CalcShortestPath(DubinsPath* path, double q0[3], double q1[3], double rho) {
        int i, errcode;
    ...
    int DubinsSpace::CalcPath(DubinsPath* path, double q0[3], double q1[3], double rho, DubinsPathType pathType){
        int errcode;
        DubinsIntermediateResults in;
        errcode = dubins_intermediate_results(&in, q0, q1, rho);
        if(errcode == EDUBOK) {
    ...
    double DubinsSpace::CalcPathLength( DubinsPath* path ) {
        double length = 0.;
        length += path->param[0];
        length += path->param[1];
        length += path->param[2];
    ...
    double DubinsSpace::CalcSegmentLength( DubinsPath* path, int i ){
        if( (i < 0) || (i > 2) ){
            return INFINITY;
        }
    ...
    double DubinsSpace::SegmentLengthNormalized(DubinsPath* path, int i ){
        if( (i < 0) || (i > 2) ){
            return INFINITY;
        }
        return path->param[i];
    ...
    DubinsSpace::DubinsPathType DubinsSpace::GetPathType(DubinsPath* path) const{
        return path->type;
    }

    int DubinsSpace::SampleOnePath( DubinsPath* path, double t, double q[3]) {
        ...
        p1 = path->param[0];
        p2 = path->param[1];
        Dubins::dubins_segment( p1,      qi,    q1, types[0] );
        Dubins::dubins_segment( p2,      q1,    q2, types[1] );
        if( tprime < p1 ) {
            Dubins::dubins_segment( tprime, qi, q, types[0] );
        }
        else if( tprime < (p1+p2) ) {
            Dubins::dubins_segment( tprime-p1, q1, q,  types[1] );
        }
        else {
            Dubins::dubins_segment( tprime-p1-p2, q2, q,  types[2] );
        }
    ...
    int DubinsSpace::SamplePaths(DubinsPath* path, double stepSize, 
                            DubinsPathSamplingCallback cb, void* user_data){
        int retcode;
        double q[3];
        double x = 0.0;
        double length = CalcPathLength(path);
        while( x <  length ) {
            SampleOnePath( path, x, q );
    ...
    int DubinsSpace::CalcPathEndpoint(DubinsPath* path, double q[3]){
        return SampleOnePath( path, CalcPathLength(path) - EPSILON, q );
    }


    int DubinsSpace::ExtractSubpath( DubinsPath* path, double t, DubinsPath* newpath ) {
